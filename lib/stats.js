...
function calcObp(hit, bb, ab, sf = 0) {
  const d = ab + bb + sf; return d ? (hit + bb) / d : null;
}
...
export function aggregateBatting(statRows) {
  ...
  const obp = calcObp(hit, bb, ab, sf);
...
export const QUALIFIED_PA_PER_GAME = 1.5;   // 1試合あたりの規定打席
export const QUALIFIED_IP_PER_GAME = 1;   // 1試合あたりの規定投球回（innings）

/** 規定打席が必要な打撃率系スタッツ */
const BAT_STATS_NEED_QUALIFIED_PA = new Set(['avg', 'obp', 'slg', 'ops']);

/** 規定投球回が必要な投手率系スタッツ */
const PIT_STATS_NEED_QUALIFIED_IP = new Set(['era', 'win_pct']);
...
export function buildRankings(players, allStats, games, year) {
  ...
  for (const p of players) {
    ...
    const hasBat = rows.some((r) =>
      (r.ab ?? 0) > 0 || (r.bb ?? 0) > 0 || (r.bunt ?? 0) > 0 || (r.sf ?? 0) > 0
    );
    if (hasBat) {
      const bat = aggregateBatting(rows);
      batData.push({ ...p, ...bat, qualified: bat.pa >= qualifiedPA });
    }
  ...
  const BAT_CATS = [
    ...
    { key:'pa',        label:'打席数',     val:(r)=>r.pa,         fmt:String,                        desc:true,  needsQualified:false },
  ...
  const PIT_CATS = [
    { key:'era',        label:'防御率',   val:(r)=>r.era,        fmt:fmtEra,                        desc:false, needsQualifiedIP:true,  allowZero:true },
    ...
    { key:'lose',       label:'敗戦',     val:(r)=>r.lose,       fmt:String,                        desc:true,  needsQualifiedIP:false },
  ...
  function makeRanking(data, cats) {
    ...
    for (const cat of cats) {
      const needsQualFilter = cat.needsQualified || cat.needsQualifiedIP;

      let qualPool = data;
      if (cat.needsQualified)   qualPool = data.filter((r) => r.qualified);
      if (cat.needsQualifiedIP) qualPool = data.filter((r) => r.qualifiedIP);
      if (cat.needsRisp)        qualPool = data.filter((r) => (r.risp_ab ?? 0) > 0);
      ...
      const valueOk = (r) => {
        ...
        if (v !== 0 || cat.allowZero) return true;
        // 規定打席達成者は打撃成績0でもランキングに表示（.000打率・0本塁打など）
        if (r.qualified && !cat.needsQualifiedIP) return true;
        return false;
      };

      const sortedQualified = [...qualPool].filter(valueOk).sort(sortFn);
      const qualifiedRows = sortedQualified.map((r, i) => ({
        rank: i + 1,
        name: r.name,
      ...
      let unqualifiedRows = [];
      if (needsQualFilter) {
        const unqualPool = data.filter((r) =>
          cat.needsQualified ? !r.qualified : !r.qualifiedIP
        );
        const sortedUnqualified = [...unqualPool].filter(valueOk).sort(sortFn);
        unqualifiedRows = sortedUnqualified.map((r) => ({
          rank: null,
          name: r.name,
          jersey_num: r.jersey_num,
          jersey_double_zero: r.jersey_double_zero,
      ...
      result[cat.key] = { qualified: qualifiedRows, unqualified: unqualifiedRows };
    }
    return result;
  }
...
export function buildTeamRecord(games, year) {
  ...
  const winPct = decided ? (win / decided).toFixed(3).replace(/^0/, '') : '—';
...
export function buildAllAwards(players, allStats, games, currentYear) {
  ...
  for (const year of years) {
    ...
    for (const p of players) {
      ...
      if (hasBat) {
        const bat = aggregateBatting(rows);
        batData.push({ name: p.name, ...bat, qualified: bat.pa >= qualifiedPA });
      }
      if (rows.some((r) => r.ip_outs > 0 || r.decision)) {
        const pit = aggregatePitching(rows);
        pitData.push({ name: p.name, ...pit, qualifiedIP: pit.ipOuts >= qualifiedIPOuts });
      }
    }
    ...
    function processBatTitle(titleMap, giveBadge) {
      for (const [key, label] of Object.entries(titleMap)) {
        let eligible = batData;
        if (BAT_STATS_NEED_QUALIFIED_PA.has(key)) {
          eligible = batData.filter((r) => r.qualified);
        }
    ...
    function processPitTitle(titleMap, giveBadge) {
      for (const [key, label] of Object.entries(titleMap)) {
        const desc = key !== 'era';
        const eligible = PIT_STATS_NEED_QUALIFIED_IP.has(key)
          ? pitData.filter((r) => r.qualifiedIP)
          : pitData;
        const sorted = [...eligible]
          .filter((r) => r[key] != null && (key === 'era' ? r[key] >= 0 : r[key] > 0))
