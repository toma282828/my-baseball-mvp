...
export function buildAllAwards(players, allStats, games, currentYear) {
  const BAT_TITLE = {
    avg:'首位打者', hr:'本塁打王', rbi:'打点王', sb:'盗塁王', hit:'最多安打', tb:'塁打王',
  };
  // エース計算対象の投手タイトル（セーブ王・最優秀中継ぎは除外）
  const PIT_TITLE_ACE = {
    era:'防御率王', win:'最多勝', so:'奪三振王', ipOuts:'投球回王',
    win_pct:'勝率王', cg:'完投王', sho:'完封王',
  };
  ...
  for (const year of years) {
    ...
    for (const [key, label] of Object.entries(BAT_TITLE)) {
      const sorted = [...batData].filter((r)=>r[key]!=null&&r[key]>0).sort((a,b)=>b[key]-a[key]);
    ...
    // エース対象投手タイトル
    for (const [key, label] of Object.entries(PIT_TITLE_ACE)) {
      const desc = key !== 'era';
      const eligible = key === 'era' || key === 'win_pct'
        ? pitData.filter((r) => r.ipOuts >= qualifiedIPOuts)
        : pitData;
      const sorted = [...eligible].filter((r)=>r[key]!=null&&r[key]>0)
    ...
    if (hpSorted.length) {
      ...
      hpSorted.filter((r)=>r.hp===top).forEach((r)=>
        addAward(r.name, year, `${year} 最優秀中継ぎ`, true, 'relief'));
    }
  }
