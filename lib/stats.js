 ...
 */
/** 規定打席の定数（1試合あたりの打席数） */
export const QUALIFIED_PA_PER_GAME = 2;
...
export function buildRankings(players, allStats, games, year) {
  ...
  const PIT_CATS = [
    { key: 'era',    label: '防御率', val: (r) => r.era,    fmt: fmtEra, desc: false, needsQualified: false },
    { key: 'win',    label: '勝利',   val: (r) => r.win,    fmt: String, desc: true,  needsQualified: false },
    { key: 'lose',   label: '敗戦',   val: (r) => r.lose,   fmt: String, desc: false, needsQualified: false },
    { key: 'so',     label: '奪三振', val: (r) => r.so,     fmt: String, desc: true,  needsQualified: false },
    { key: 'ipOuts', label: '投球回', val: (r) => r.ipOuts, fmt: outsToIp, desc: true, needsQualified: false },
    { key: 'sv',     label: 'セーブ', val: (r) => r.sv,     fmt: String, desc: true,  needsQualified: false },
  ];

  function makeRanking(data, cats) {
    const result = {};
    for (const cat of cats) {
      const eligible = cat.needsQualified
        ? data.filter((r) => r.qualified)
        : data;
