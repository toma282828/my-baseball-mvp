...
export async function getAppData(teamSlug) {
  ...
  const [playersRes, gamesRes, teamRes] = await Promise.all([
    ...
    supabase.from('teams').select('display_name').eq('slug', teamSlug).maybeSingle(),
  ]);
  ...
  return {
    ...
    games,
    stats,
    teamName: teamRes.data?.display_name || teamSlug,
  };
}
